...
if RemInt(sampleCounter, 100000) = 0 then
# evaluation -- can be removed in general
tmpMargs := [];;
for i in [1..sizeOfElements] do
tmpMargs[i] := Float(marginals[i]) / Float(sampleCounter);;
od;;
Print((runtimeSum()-sampleTimer)/1000.0+buildHashTime, " ", kullback(tmpMargs, correctMargs, 2), "\n");;
fi;;
od;;
